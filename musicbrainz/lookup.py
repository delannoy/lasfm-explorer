...
def lookup(entity: str, mbid: uuid.UUID, inc: list[str] = [], type: list[str] = [], status: list[str] = []) -> awkward.Record:
...
def area(mbid: uuid.UUID, inc: list[str] = []) -> awkward.Record:
    return lookup(entity='area', mbid=mbid, inc=inc)

def artist(mbid: uuid.UUID, inc: list[str] = [], type: list[str] = [], status: list[str] = []) -> awkward.Record:
    return lookup(entity='artist', mbid=mbid, inc=inc, type=type, status=status)

def collection(mbid: uuid.UUID, inc: list[str] = []) -> awkward.Record:
    return lookup(entity='collection', mbid=mbid, inc=inc)

def event(mbid: uuid.UUID, inc: list[str] = []) -> awkward.Record:
    return lookup(entity='event', mbid=mbid, inc=inc)

def genre(mbid: uuid.UUID, inc: list[str] = []) -> awkward.Record:
    return lookup(entity='genre', mbid=mbid, inc=inc)

def instrument(mbid: uuid.UUID, inc: list[str] = []) -> awkward.Record:
    return lookup(entity='instrument', mbid=mbid, inc=inc)

def label(mbid: uuid.UUID, inc: list[str] = [], type: list[str] = [], status: list[str] = []) -> awkward.Record:
    return lookup(entity='label', mbid=mbid, inc=inc, type=type, status=status)

def place(mbid: uuid.UUID, inc: list[str] = []) -> awkward.Record:
    return lookup(entity='place', mbid=mbid, inc=inc)

def recording(mbid: uuid.UUID, inc: list[str] = [], type: list[str] = [], status: list[str] = []) -> awkward.Record:
    return lookup(entity='recording', mbid=mbid, inc=inc, type=type, status=status)

def release(mbid: uuid.UUID, inc: list[str] = [], type: list[str] = []) -> awkward.Record:
    return lookup(entity='release', mbid=mbid, inc=inc, type=type)

def release_group(mbid: uuid.UUID, inc: list[str] = [], type: list[str] = [], status: list[str] = []) -> awkward.Record:
    return lookup(entity='release-group', mbid=mbid, inc=inc, type=type, status=status)

def series(mbid: uuid.UUID, inc: list[str] = []) -> awkward.Record:
    return lookup(entity='series', mbid=mbid, inc=inc)

def url(mbid: uuid.UUID = None, inc: list[str] = []) -> awkward.Record:
    return lookup(entity='url', mbid=mbid, inc=inc)

def work(mbid: uuid.UUID, inc: list[str] = []) -> awkward.Record:
    return lookup(entity='work', mbid=mbid, inc=inc)

# [Non-MBID Lookups](https://musicbrainz.org/doc/MusicBrainz_API#Non-MBID_Lookups)

def discid(discid: str, inc: list[str] = [], toc: list[int] = [], cdstubs: bool = True, media_format: str = 'CD') -> awkward.Record:
    validateIncludes(inc=inc, entity='discid')
...
def isrc(isrc: str, inc: list[str] = []) -> awkward.Record:
    validateIncludes(inc=inc, entity='isrc')
    return request.get(endpoint=f'/isrc/{isrc}', inc=str.join('+', inc))

def iswc(iswc: str, inc: list[str] = []) -> awkward.Record:
    validateIncludes(inc=inc, entity='iswc')
